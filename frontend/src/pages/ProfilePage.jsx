          ...
          {(!userInfo.wishlist || userInfo.wishlist.length === 0) ? (
            ...
            <ul className="grid gap-4">
              {userInfo.wishlist.map((productId) => (
                <li key={productId} className="flex justify-between items-center p-4 border border-gray-700 rounded-lg hover:shadow-md transition-shadow bg-gray-700">
                  <Link to={`/product/${productId}`} className="text-gray-200 hover:text-blue-400">
                    {productId}
                  </Link>
                  <button
