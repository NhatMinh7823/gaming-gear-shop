...
export const useWishlist = () => {
  ...
  const refreshWishlist = async (forceRefresh = false) => {
    ...
    const now = Date.now();
    if (!forceRefresh && lastGlobalFetchTime > 0 && now - lastGlobalFetchTime < 60000) {
        ...
        if (!wishlistLoadedRef.current) {
          try {
            ...
            if (data.success && data.wishlist) {
              ...
              dispatch(updateWishlist(wishlistIds));
              dispatch(setWishlist(wishlistIds));
              console.log('Wishlist loaded successfully:', wishlistIds.length, 'items');
            } else {
            ...
            console.error('Failed to load wishlist:', error.response?.data?.message || error.message);
  ...
  useEffect(() => {
    ...
    } else if (userInfo && (!userInfo.wishlist || userInfo.wishlist.length === 0) && wishlistItems.length > 0) {
      // If wishlistItems has items but userInfo.wishlist is empty, sync them
      dispatch(updateWishlist(wishlistItems));
    }
  }, [userInfo?.wishlist?.length, wishlistItems.length, userInfo, wishlistItems, dispatch]);
