...
import { ADDRESS, ORDER } from '../utils/toastMessages';
import {
  createOrder,
  clearCart,
  markCouponAsUsed,
...
export const useCheckout = () => {
  ...
  const handleNextStep = () => {
    if (activeStep === 1) {
      ...
      if (!validateShippingForm()) {
        toast.error(ADDRESS.SHIPPING_CHECK_ERROR);
        return;
      }
      setActiveStep(3);
      window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  const handleCheckout = async (appliedCoupon, discountAmount) => {
    ...
    try {
      ...
      const finalTotal =
        totalPrice + finalShippingFee + TAX_PRICE - discountAmount;
      ...
      if (
        appliedCoupon &&
        appliedCoupon.code &&
        !appliedCoupon.code.match(
          /^(SAVE10LOW|SAVE10MID|SAVE10HIGH|SAVE20MID|SAVE20HIGH|SAVE20PREMIUM|FREESHIP)$/
        )
      ) {
